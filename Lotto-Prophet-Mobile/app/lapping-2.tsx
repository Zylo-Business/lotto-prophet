...
const NUM_PATTERN_ROWS = 2;

function getDefaultPattern(mode: ColumnsMode): string[][] {
  const numCols = mode === 'all' ? 10 : 5;
  return Array.from({ length: NUM_PATTERN_ROWS }, () => new Array(numCols).fill(''));
}
...
export default function Lapping2() {
  ...
  // ── Column mode change (resets pattern) ──
  ...
  // ── Grid cell edit ──
  const updateCell = useCallback((rowIndex: number, colIndex: number, text: string) => {
  ...
  // ── Run analysis (reads pattern from ref to avoid stale closure) ──
      ...
      <>
        ...
        >
          ...
          <Text style={styles.patternHint}>
            Enter numbers (1–90) per column to search for a specific 2-row pattern.
            {'\n'}Leave empty to detect all two-lapping.
          </Text>
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.crossCardStats}>
                    ...
                    <Text style={styles.crossCardStat}>
                      Hits: <Text style={[styles.crossCardStatBold, { color: COLORS.primary }]}>{hits}</Text>
                    </Text>
                    <Text style={styles.crossCardStat}>
                      Rate: <Text style={[styles.crossCardStatBold, { color: COLORS.primary }]}>{rate}%</Text>
                    </Text>
                  </View>
                </TouchableOpacity>
              );
              ...
              <View style={styles.controlCard}>
                <Text style={styles.noResultsText}>
                  {hasPatternValues
                    ...
                    : 'Not enough draws for lapping-2 analysis (minimum 2 required).'}
                </Text>
              </View>
            ) : null
          }
...
const createStyles = (COLORS: AppColors) =>
  StyleSheet.create({
    ...
    statBadgePattern: { backgroundColor: '#F5F3FF', borderColor: '#C4B5FD' },
    ...
    statLabelPattern: { fontSize: 11, color: '#6D28D9' },
    statValuePattern: { fontSize: 14, fontWeight: '600', color: '#6D28D9' },
    ...
    drawCardLapping: { borderLeftWidth: 3, borderLeftColor: COLORS.primary },
    drawCardHeader: {
      flexDirection: 'row',
    ...
    ballNormal: { backgroundColor: `${COLORS.primary}15` },
    ballHighlight: {
    ...
    sideNavItemActive: { backgroundColor: `${COLORS.primary}20` },
    sideNavText: { fontSize: 11, fontWeight: '500', color: COLORS.textSecondary },
    sideNavTextActive: { color: COLORS.primary, fontWeight: '700' },
