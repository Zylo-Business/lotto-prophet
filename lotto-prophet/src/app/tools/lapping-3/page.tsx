...
const DEFAULTS_MAIN: string[][] = [
  ["61", "60", "17", "8", "75"],
  ["55", "28", "11", "6", "47"],
  ["53", "29", "22", "75", "82"],
];
const DEFAULTS_MACHINE: string[][] = [
  ["61", "60", "17", "8", "75"],
  ["55", "28", "11", "6", "47"],
  ["53", "29", "22", "75", "82"],
];
...
function getDefaultPattern(mode: ColumnsMode): string[][] {
  if (mode === "machine") return DEFAULTS_MACHINE.map((r) => [...r]);
  if (mode === "all")
    return DEFAULTS_MAIN.map((r) => [
      ...r,
      ...new Array(5).fill(""),
    ]);
  return DEFAULTS_MAIN.map((r) => [...r]);
}

function getEmptyPattern(mode: ColumnsMode): string[][] {
...
export default function Lapping3Page() {
  ...
  const clearPattern = useCallback(() => {
    setPatternRows(getEmptyPattern(columnsMode));
  }, [columnsMode]);
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        <span>/</span>
        <span className="text-foreground font-medium">Lapping 3</span>
      </nav>
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Scans 3 consecutive rows per column — if bottom matches row 1, middle
          matches row 2, and top matches row 3, the entire column is highlighted.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-4 mb-6 rounded-xl border border-border bg-card p-4 shadow-sm">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select
            className="rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-amber-500"
      ...
      <div className="rounded-xl border border-border bg-card p-4 mb-6 shadow-sm">
        ...
        {result && result.grid.length >= NUM_ROWS && (
          <div className="flex items-center gap-2 mb-3">
            ...
            <select
              className="rounded-lg border border-border bg-background px-3 py-1.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-amber-500 max-w-xs"
              value=""
              onChange={(e) => {
                  ...
                  {row.map((val, ci) => (
                    <td key={ci} className="px-1 py-1">
                      <Input
                        ...
                        className="w-14 h-9 text-center text-sm tabular-nums focus:ring-amber-500"
                        placeholder="–"
                        maxLength={2}
                      />
                    </td>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {isPrimary && (
                      <span className="text-[10px] uppercase tracking-wide font-bold text-amber-500 bg-amber-100 dark:bg-amber-900/50 px-1.5 py-0.5 rounded">
                        Primary
                      </span>
                    )}
                  </div>
      ...
      {result && (
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <div className="rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 px-4 py-2 text-sm shadow-sm">
              ...
              <tbody>
                {result.draws.map((draw, ri) => {
                  ...
                  return (
                    ...
                    >
                      <td className={`px-3 py-2 tabular-nums sticky left-0 z-10 ${isLapping ? "bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300 font-semibold" : "bg-card text-muted-foreground"}`}>
                        {ri + 1}
                      </td>
                      <td className={`px-3 py-2 font-medium tabular-nums ${isLapping ? "text-amber-700 dark:text-amber-300" : "text-foreground"}`}>
                        {draw.event_number}
                      </td>
                      <td className={`px-3 py-2 whitespace-nowrap ${isLapping ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground"}`}>
      ...
      {result && result.grid.length === 0 && !loading && (
        <div className="rounded-xl border border-border bg-card p-10 text-center shadow-sm">
          <p className="text-muted-foreground text-lg">
            Not enough draws to perform lapping-3 analysis (minimum 3 required).
          </p>
        </div>
      )}
